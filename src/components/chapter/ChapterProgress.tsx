...
const ChapterProgress = ({ totalStep, step, onClickStep }: Props) => {
    ...
    return (
        <div
            className={cn(
                "transition-all group pb-1 fixed flex items-center bottom-0 pt-6 hover:pt-0 hover:h-[var(--chapter-progress-height)] hover:bg-background hover:border-t hover:border-t-accent left-0 w-full px-[var(--side-margin)]",
        ...
        before:absolute before:top-0  before:z-[1] before:pointer-events-none before:opacity-50 before:h-full before:transition-all before:rounded-full before:w-[var(--current-progress)] before:bg-primary`,
                ...
                style={
                    {
                        [`--current-progress`]: `${progressPercentage - 4}px`,
                    } as MyCustomCSS
                }
            >
