...
const Navbar = (props: Props) => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            const opacity = Math.min(window.scrollY / displayPoint, 1).toFixed(1)
    ...
    return (
        <div className='h-[var(--navbar-height)] w-full fixed top-0 left-0 z-[var(--navbar-index)]'>
            <div ref={navbarBgRef} className='absolute transition-opacity opacity-0 top-0 left-0 right-0 bottom-0 bg-background border-b border-primary'></div>
            <Wrapper className={cn("flex items-center h-full relative z-[2]")}>
                <Button variant={"outline"} size={"icon"} className='rounded-full text-2xl' onClick={() => drawerMenu.setIsActive?.(true)}>
